...
# 2. Modify for Python3 and add catch exceptions (try, except, finally)
...
class root(Tkinter.Tk):
    ...
    def initialize(self):   # build the main window
        ...
        button = Tkinter.Button(self,text=u"Spend", command=self.OnButtonSpend)
        button.grid(column=3,row=0,sticky='EW')
        button = Tkinter.Button(self,text=u"Setup", command=self.OnButtonSetup)
        button.grid(column=2,row=1,sticky='EW')
        button = Tkinter.Button(self,text=u"Increment", command=self.OnButtonIncrement)
        button.grid(column=3,row=1,sticky='EW')
        button = Tkinter.Button(self,text=u"Clear Data", command=self.OnButtonClear)
        ...
        if isfile('data.txt'):
            increment(readData())
            self.amount.set("Available:  ${0:.2f}".format(readData()['funMoney']))
        else: self.amount.set("Run Setup")
    ...
    def OnButtonAdd(self):   # Add money to the total
        ...
        if check():
            if adjustMoney(readData(), amount, 1):
                ...
                tkMessageBox.showwarning("Bad input", "Invalid amount!")
                self.labelVariable.set("You added:  $0")
            self.amount.set("Available:  ${0:.2f}".format(readData()['funMoney']))
        else: self.labelVariable.set("No data available")
        self.entry.focus_set()
        self.entry.selection_range(0, Tkinter.END)
    ...
    def OnButtonIncrement(self):   # Run SetIncrement()
        ...
        if check():
            if SetIncrement(readData(), amount):
                self.labelVariable.set("${0:.2f}".format(float(amount))+"/m = ${0:.2f}".format(readData()['increment'])+"/d")
            else:
                tkMessageBox.showwarning("Bad input", "Invalid amount!")
...
def setup():  # Sets start date based on user input
    d = MyDialog(app)
    try:
        newDate = datetime.datetime(int(d.startyear),int(d.startmonth),int(d.startday))
        test = True
    except AttributeError:
        test = False
    except (ValueError, TypeError):
        tkMessageBox.showwarning("Bad input", "Invalid date!")
    ...
    if test:
        ...
        info = {'start':start,'today':today,'totalDays':totalDays,'totalMoney':totalMoney,'spentMoney':spentMoney,'funMoney':funMoney, 'increment':increment}
        writeData(info)
...
def increment(info):  # Increments total money available based on today's date
    if info['today'] != str(datetime.date.today()):
        ...
        dateDiff = datetime.date.today() - datetime.date(info['start'][2],info['start'][1],info['start'][0])
...
def adjustMoney(info, amount, kind):  # adds or subtracts money from amount available
    try:
        isinstance(float(amount), float)
    except ValueError:
        return False
